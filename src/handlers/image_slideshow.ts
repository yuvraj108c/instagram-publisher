...
async function createImageSlideshowHandler(
  ...
  for (let idx = 0; idx < images.length; idx++) {
    ...
    try {
      const uploadResponse = await _uploadPhoto({ photo, http_client });
...
function _getRandomArbitrary(min: number, max: number): Number {
  return Math.random() * (max - min) + min;
}

async function _saveSlideshow({ photosUploaded, caption, http_client }: any) {
  const payload = {
    caption,
  ...
  const uploadResponse = await http_client({
...
async function _uploadPhoto({
  ...
  const nameEntity = `${uploadId}_0_${_getRandomArbitrary(
    1000000000,
    9999999999
  )}`;
  ...
  const headersPhoto = {
    ...
    'X-IG-Connection-Speed': `${_getRandomArbitrary(1000, 3700)}kbps`,
  ...
  let responseUpload = await http_client({
